...
function Arena({ go, mode, player, opponent, environment, result, seed, preferences, updatePreferences }: { go:(screen:Screen)=>void;mode:GameMode;player:Fighter;opponent:Fighter;environment:Environment;result:BattleResult;seed:number;preferences:GamePreferences;updatePreferences:(patch:Partial<GamePreferences>)=>void }) {
...
export default function HomePage() {
  ...
  const [,setHistory]=useState<Screen[]>([]); const [preferences,setPreferences]=useState<GamePreferences>(DEFAULT_PREFERENCES); const [viewport,setViewport]=useState<ViewportClass>("desktop"); const [tutorialActive,setTutorialActive]=useState(false);
  ...
  const content = screen === "home" ? <Home start={startGame} mode={mode} setMode={setMode} open={navigate} introSeen={preferences.introSeen||preferences.reducedMotion} skipIntro={()=>updatePreferences({introSeen:true})} ready={!catalogLoading&&catalog.length>0}/> :
    screen === "settings" ? <SettingsScreen preferences={preferences} update={updatePreferences} replayIntro={()=>{updatePreferences({introSeen:false});setHistory([]);setScreen("home")}} replayTutorial={()=>{updatePreferences({tutorialSeen:false});setTutorialActive(true);setMode("1_player");setHistory([]);setScreen("home")}}/> :
